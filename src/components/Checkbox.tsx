...
export function Checkbox({ title, isChecked, onCheckedChange }: Props) {
  return (
    ...
    >
      ...
      <span className="font-semibold text-xl text-white group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400 whitespace-nowrap overflow-hidden text-ellipsis mr-2">
        {title}
      </span>
    </CheckBoxRadix.Root>
  )
